...
public class QwintoMatch {
	private QwinPlayer[] player;
	private QwinDice dice;
	private int currentPlayerIndex;
	private boolean matchEnd;
	
	public QwintoMatch(QwinPlayer... initPlayer) {
		this(new Random(), initPlayer);
	}
	
	public QwintoMatch(Random rnd, QwinPlayer... initPlayer) {
		player = initPlayer;
	...
	public void currentPlayerTurn(boolean print) {
		if (print) System.out.println("Turn for player "+currentPlayerIndex+" ("+player[currentPlayerIndex].getName()+")");
		QwinPlayer p = player[currentPlayerIndex];
		//Throw Dice
		DiceThrow th = p.getDiceThrow();
		...
		int[] actionList = p.getActionFlagList(thrown, dice.getLastThrown());
		if (paperEnterNumber(p.getPaper(), thrown, actionList, true, print) == 0) { // rethrow dice
			...
			if (print) System.out.println("the sum of the "+th.getNumberOfDice()+" dice is "+thrown+".");
			actionList = p.getActionFlagList(thrown, dice.getLastThrown());
